...
void MovingHead::serialize(nlohmann::json & json) {
	...
	// fixtureSettings
	{
		...
		// dmxAddresses
		{
			...
			json["fixtureSettings"]["dmxAddresses"] << this->fixtureSettings.dmxAddresses.brightnessCoarse;
            json["fixtureSettings"]["dmxAddresses"] << this->fixtureSettings.dmxAddresses.brightnessFine;
...
void MovingHead::populateWidgets(shared_ptr<ofxCvGui::Panels::Widgets> widgets) {
	...
	widgets->addTitle("Fixture settings", ofxCvGui::Widgets::Title::Level::H2);
	{
		...
		widgets->addTitle("DMX Addresses", ofxCvGui::Widgets::Title::Level::H3);
		{
			...
			widgets->addEditableValue<uint16_t>(this->fixtureSettings.dmxAddresses.brightnessCoarse);
            widgets->addEditableValue<uint16_t>(this->fixtureSettings.dmxAddresses.brightnessFine);
	...
	// Navigate the focus (NOTE : hardcoded for Robe Pointe)
	{
		...
		if (distance < 1.5f) {
			...
			auto dmxCoarseFloat =
				...
				+ 29.67284246;
//            this->beamParameters.focus.set(dmxCoarseFloat / 256.0f); //stephan÷
		}
	}
}
...
void MovingHead::renderDMX(vector<uint8_t> & dmxValues) const {
    ...
    int temp = this->fixtureSettings.dmxAddresses.panCoarse.get();
//    ofLog()<<"dmxOffset "<<dmxOffset<<" temp "<<temp<<" dmxOffset + this->fixtureSettings.dmxAddresses.panCoarse.get() "<<dmxOffset + temp;
	...
	dmxValues[dmxOffset + this->fixtureSettings.dmxAddresses.brightnessCoarse.get()] = 255;
    dmxValues[dmxOffset + this->fixtureSettings.dmxAddresses.brightnessFine.get()] = 255;
	...
	//focus
	{
		...
		dmxValues[dmxOffset + this->fixtureSettings.dmxAddresses.focusFine.get()] = focusFine; //stephan
	}
    
//   int dmx_focus = 127;
//    int dmx_coarseDimmer = 127; //30;
    
//    dmxValues[dmxOffset + 22] = 255;  //strobe stop
//    dmxValues[dmxOffset + 23] = dmx_coarseDimmer;  ////DIMMER
//    dmxValues[dmxOffset + 10] = 64; ////static gobo
//    dmxValues[dmxOffset + 17] = 255;   //zoom
//    dmxValues[dmxOffset + 5] = 30; //pan tilt speed
